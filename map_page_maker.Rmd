...
##census data
# key <- "a2493c786b7709e2824f37d355bf1199568399d1"
# 
...
# # #
#  save(county_demos, file="~/1TB/corona_map_maker/county_demos.rda")

load("~/1TB/corona_map_maker/county_demos.rda")
...
nyt_demo <- merge(nyt_data, county_demos, by.x="fips",by.y="GEOID") %>% select(-moe) %>%
  ...
  mutate(cumul_cases_1k = (sum(cases)/population)*1000,
         cumul_deaths_1k = (sum(deaths)/population)*1000) %>%
  rename(GEOID=fips) 
...
#save(map_shape, file="~/1TB/corona_map_maker/county_maps.rda")
load("~/1TB/corona_map_maker/county_maps.rda")
...
# fcs <- fast_counties %>% ggplot(aes(cases, double_time, size=population, color=state, label=NAME)) + geom_point() + theme_bw() +
#   theme(legend.position="none") + ylab("Doubling Time, (Days)") + xlab("Cases") + ylim(0,max(fast_counties$double_time)) 
# 
# ggplotly(fcs, tooltip = c("NAME","cases","double_time","population"))
